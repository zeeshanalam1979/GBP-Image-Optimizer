...
def inject_metadata(img_bytes, row, ext):
    """Strip all metadata then inject from CSV row. Returns processed bytes."""
    ext = ext.lower().lstrip('.')

    img = Image.open(io.BytesIO(img_bytes))

    # Strip: create clean copy without metadata
    ...
    # Keywords → XPKeywords (Windows) & UserComment
    keywords = str(row.get("keywords", ""))
    if keywords:
        kw_bytes = keywords.encode("utf-16-le")
        exif_dict["0th"][piexif.ImageIFD.XPKeywords] = kw_bytes
    ...
    if save_fmt == "JPG":
        save_fmt = "JPEG"
    try:
        img_clean.save(out_buf, format=save_fmt, exif=exif_bytes)
    except Exception:
        img_clean.save(out_buf, format=save_fmt)
        ...
        if st.button("🚀 Inject Metadata & Download", key="single_go"):
            if not single_img:
                ...
                with st.spinner("Processing…"):
                    ...
                    ext  = single_img.name.rsplit(".", 1)[-1]
                    fname = build_filename(s_biz, s_svc, s_loc, s_date.strftime("%Y-%m-%d"), 1, ext)
                    out_bytes = inject_metadata(single_img.read(), row, ext)

                st.success(f"✅ Done! Output: `{fname}`")
                st.download_button(
                    "⬇️ Download Optimized Image", data=out_bytes,
                    file_name=fname, mime=f"image/{ext}",
                    key="single_dl"
                )
            ...
            if matched_count == 0:
                ...
                with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
                    for idx, img_file in enumerate(batch_imgs):
                        ...
                        try:
                            img_file.seek(0)
                            out_bytes = inject_metadata(img_file.read(), csv_row, ext)
                ...
                if processed > 0:
                    st.success(f"✅ {processed} images processed successfully!")
                    st.download_button(
