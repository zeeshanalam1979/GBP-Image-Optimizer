...
# ── Page config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Enahnce Local SEO with GeoRank Pro Image Exif Injection Tool",
    page_icon="🌐",
    layout="wide",
...
def inject_metadata(img_bytes, row, ext, output_format=None):
    ...
    if out_fmt == "JPEG":
        ...
        # Keywords → XPKeywords only (Windows Tags field)
        keywords = str(row.get("keywords", ""))
        if keywords:
            exif_dict["0th"][piexif.ImageIFD.XPKeywords] = keywords.encode("utf-16-le")

        # Comments → XPComment (Windows 11 native) + UserComment (EXIF standard)
        # XPComment is the field Windows 11 File Properties "Comments" reads directly
        comments = str(row.get("comments", "") or row.get("description", ""))
        if comments:
            # XPComment — UTF-16-LE, read natively by Windows Explorer
            exif_dict["0th"][piexif.ImageIFD.XPComment] = comments.encode("utf-16-le")
            # UserComment — EXIF standard with ASCII header (avoids Chinese character bug)
            # Use ASCII header + UTF-8 body for maximum compatibility
            safe_comment = comments.encode("utf-8", errors="replace")
            exif_dict["Exif"][piexif.ExifIFD.UserComment] = b"ASCII\x00\x00\x00" + safe_comment
    ...
    try:
        ...
        if exif_data:
            ...
            if piexif.ImageIFD.XPKeywords in ifd0:
                result["Keywords"] = ifd0[piexif.ImageIFD.XPKeywords].decode("utf-16-le", errors="replace").rstrip("\x00")
            if piexif.ImageIFD.XPComment in ifd0:
                result["Comments"] = ifd0[piexif.ImageIFD.XPComment].decode("utf-16-le", errors="replace").rstrip("\x00")
            elif piexif.ExifIFD.UserComment in exif_dict.get("Exif", {}):
                raw = exif_dict["Exif"][piexif.ExifIFD.UserComment]
                result["Comments"] = raw[8:].decode("utf-8", errors="replace").strip("\x00")
...
<div class="hero-banner">
    <div class="hero-title">GeoRank <span>Pro</span></div>
    <div class="hero-sub">GPS · Metadata · Format Conversion · Built for Google Business Profile SEO</div>
</div>
""", unsafe_allow_html=True)
...
with tab3:
    ...
    st.markdown('<div class="section-label">Verify injected metadata on any image before uploading to GBP — powered by GeoRank Pro</div>', unsafe_allow_html=True)
